...
if __name__ == '__main__':
    url = easygui.enterbox(title="YOUTUBE URL", msg="Enter Youtube URL: ")
    setup(url)
...
if __name__ == '__main__':
    easygui.msgbox(title="yt.info", msg="URL: " + url + "\nCreator: " + creator(
        setup(url)) + "\nViews: " + views(
        setup(url)) + "\nSubscribers: " + subs(setup(url)) + "\nLikes: " + likes(
        setup(url)) + "\nDislikes: " + dislikes(
        setup(url)) + "\n\nADDITIONAL yt.info:\n\nIs Private: " +is_private(
        setup(url)) + "\nAverage Ratings: " + average_rating(setup(url)) + "\n")
